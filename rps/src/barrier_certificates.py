...
def create_single_integrator_barrier_certificate_time_varying(Delta, lamb,target_shape, t, safety_radius=0.17, barrier_gain=100, safety_a=0.17, safety_b=0.12, magnitude_limit=0.2):
    ...
    def f(dxi, x,theta):
        ...
        ##  Delta = sin(t) 
        if Delta < 1:
            Delta_dot = np.cos(t) # compute delta dot
        else:
            Delta_dot = 0
        ...
        for i in range(N-1):
            for j in range(i+1, N):
                ...
                # Current h_dot
                h_prev_dot1 = lamb[0] * h_circ_dot1 +  lamb[1] * h_ellip_dot1
                h_prev_dot2 = lamb[0] * h_circ_dot2 +  lamb[1] * h_ellip_dot2
...
def create_single_integrator_barrier_certificate_with_obstacles(barrier_gain=100, safety_radius=0.17, magnitude_limit=0.2):
    ...
    def f(dxi, x):
        ...
        # Initialize some variables for computational savings
        N = dxi.shape[1]
        num_constraints = int(comb(N, 2)) + + N * 2
        A = np.zeros((num_constraints, 2*N))
        b = np.zeros(num_constraints)
        #H = sparse(matrix(2*np.identity(2*N)))
        H = 2*np.identity(2*N)
